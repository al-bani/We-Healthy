...
class _DataFisikState extends State<DataFisik> {
  ...
  void insertData(var bb, var tb, double metValue, var gender, var umur) async {
    bb = double.parse(bb);
    tb = double.parse(tb);
    umur = int.parse(umur);
    ...
    double tdee = calc.tdee(bmr, metValue);
    ...
    try {
      ...
      if (userData.length == 1) {
        Navigator.pushNamed(context, 'home_screen', arguments: userId);
      } else {
        setState(() {
          loading = false;
        });
